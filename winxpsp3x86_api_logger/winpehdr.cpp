/*
 * This is still for 32-bit. the structures must be updated for 64-bit windows
 */
...
namespace winpe {
    ...
    uint8_t * WinPE::_get_export_data(peaddr_t &virtual_addr, uint32_t &size) {
        ...
        virtual_addr = export_data->VirtualAddress;
        size = export_data->Size;
        
        uint8_t * data = (new uint8_t[size+10]);
        
        if (0 == this->read_mem(
                this->m_pe_base + virtual_addr,
                data,
                size)) {
    ...
    string WinPE::get_api_name_from_addr(peaddr_t addr) {
        
        
#ifdef WINPE_DEBUG
        fprintf(this->m_dump, ">> WinPE::get_api_name_from_addr(%016lx)\n", (uint64_t)addr);
#endif
        ...
        // ALGORITHM WRONG HERE!!
        
        // Correct algo is thus:
        // 1. Find the address of the possible function 
        //      (max address that is lower or equal to the given addr) (OK)
        //      let pi := find the index of that function
        //
        // 2. Find that index in ordinal table (name_ord_tbl).
        //      let xi := name_ord_tbl[xi] == pi
        //
        // 3. Return name_ptr_tbl[xi]
        
        // STEP 1
        peaddr_t max_func_addr = 0;
        uint32_t max_func_i;
        
        for (uint32_t i=0; i<imgexport_num_names; ++i) {
            
            peaddr_t fn_addr = fn_addr_tbl[i] + this->m_pe_base;
            if (fn_addr <= addr) {
                
                if (max_func_addr < fn_addr) {
                    max_func_addr = fn_addr;
                    max_func_i = i;
                }
        ...
        if (nameidx == 0xFFFFFFFF) {
            ...
            sprintf(tmp, "(ORD_0x%x)", max_func_i);
            tmp[31] = '\0';
            return string(tmp);
        }
        
        // STEP 3
        peaddr_t namepos = name_ptr_tbl[nameidx];
        char * name = (char*)&(imgexport_data[namepos - export_data->VirtualAddress]);
        return string(name);
    }
};
