...
int mem_callback(CPUState *env, target_ulong pc, target_ulong addr,
    ...
    prog_point p = {};
#ifdef TARGET_I386
	CPUArchState * cpu = reinterpret_cast<CPUArchState *>(env->env_ptr);
    panda_virtual_memory_rw(env, cpu->regs[R_EBP]+4, (uint8_t *)&p.caller, 4, 0);
    if((cpu->hflags & HF_CPL_MASK) != 0) // Lump all kernel-mode CR3s together
        p.sidFirst = cpu->cr[3];
#endif
    p.pc = pc;
...
bool init_plugin(void *self) {
    ...
    if (read_idx) {
        ...
        while (!feof(read_idx)) {
            (void) fread(&p, sizeof(p), 1, read_idx);
            (void) fread(&size, sizeof(target_ulong), 1, read_idx);
        ...
        if (!read_log)
            perror("fopen");
        (void) ftruncate(fileno(read_log), read_sz);
    ...
    if (write_idx) {
        ...
        while (!feof(write_idx)) {
            (void) fread(&p, sizeof(p), 1, write_idx);
            (void) fread(&size, sizeof(target_ulong), 1, write_idx);
        ...
        if (!write_log)
            perror("fopen");
        (void) ftruncate(fileno(write_log), write_sz);
