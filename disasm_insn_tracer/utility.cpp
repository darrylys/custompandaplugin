...
#if defined(TARGET_I386)
            if (cs_open(CS_ARCH_X86, CS_MODE_32, &this->m_handle) != CS_ERR_OK) {
#elif defined(TARGET_X86_64)
            if (cs_open(CS_ARCH_X86, CS_MODE_64, &this->m_handle) != CS_ERR_OK) {
#elif defined(TARGET_ARM)
            if (cs_open(CS_ARCH_ARM, CS_MODE_32, &this->m_handle) != CS_ERR_OK) {
#else
#error "ERROR! Unknown architecture."
        ...
        bool disasm(FILE *out, const uint8_t *code, uint32_t size, uint64_t addr, int max_ins) {
            ...
            if (count > 0) {
                ...
                //fprintf(out, "\nDISASSEMBLY:\n");
                int lim = (max_ins >= 0 && max_ins < count) ? max_ins : count;
    ...
    bool Disassembler::disasm(FILE* out, const void* code, uint32_t size, uint64_t addr, int max_ins) {
        return this->m_impl->disasm(out, (uint8_t*)code, size, addr, max_ins);
    }
