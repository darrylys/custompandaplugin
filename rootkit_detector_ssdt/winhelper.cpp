...
namespace panda {
    namespace win {
        ...
        ptr_t get_tcb_service_table(CPUState *env) {
            
            ptr_t kpcr = get_kpcr(env);
            ptr_t kthread;
            
            panda_virtual_memory_rw(env, kpcr+KPCR_CURTHREAD_OFF, (uint8_t *)&kthread, sizeof(ptr_t), false);
            
            ptr_t service_tbl;
            panda_virtual_memory_rw(env, kthread+ETHREAD_TCB_SERVICETABLE_OFF, 
                    (uint8_t *)&service_tbl, sizeof(ptr_t), false);
