...
#include "my_callstack_instr.h"

extern "C" {
#include "panda/plog.h"
#include "my_callstack_instr_int_fns.h"
...
std::set<target_ulong> target_processes;
bool is_process_analyzed(CPUArchState*, const stackid&);
bool is_process_analyzed(CPUState *);
...
void before_block_exec(CPUState *cpu, TranslationBlock *tb) {
	...
	if (!is_process_analyzed(env, curStackid)) {
		return;
	}
 ...
 */
uint32_t my_get_callers(target_ulong callers[], uint32_t n, CPUState* cpu) {
    CPUArchState* env = (CPUArchState*)cpu->env_ptr;
    std::vector<stack_entry> &v = callstacks[get_stackid(env)];
 ...
 */
uint32_t my_get_functions(target_ulong functions[], uint32_t n, CPUState* cpu) {
    CPUArchState* env = (CPUArchState*)cpu->env_ptr;
...
void my_get_prog_point(CPUState* cpu, prog_point *p) {
    CPUArchState* env = (CPUArchState*)cpu->env_ptr;
    if (!p) return;
    ...
    int n_callers = 0;
    n_callers = my_get_callers(&p->caller, 1, cpu);
...
bool parse_prog_list(const char * progListCsv, bool isAsidList) {
	
	std::vector<std::string> tmp;
	if (!parseCsv(progListCsv, '|', tmp)) {
		return false;
	}
	
	for (int i = 0; i < tmp.size(); ++i) {
		std::string& tok = tmp[i];
		
		uint64_t val = 0;
		if (isAsidList) {
			sscanf(tok.c_str(), "%lx", &val);
		} else {
			sscanf(tok.c_str(), "%lu", &val);
		}
		printf("prog_list tok [%d]: %s, parsed: %lx (%lu)\n", 
				i, tok.c_str(), val, val);
		
		target_processes.insert((target_ulong)val);
	}
...
bool is_process_analyzed(CPUArchState* env, const stackid& si) {
	target_ulong id = std::get<0>(si);
	if (!target_processes.empty()) {
		return !in_kernelspace(env) && target_processes.find(id) != target_processes.end();
	}
	return true;
}

bool is_process_analyzed(CPUState* cpu) {
	CPUArchState* env = (CPUArchState*)(cpu->env_ptr);
	stackid si = get_stackid(env);
	return is_process_analyzed(env, si);
}
...
bool init_plugin(void *self) {
    ...
    panda_arg_list *args = panda_get_args("my_callstack_instr");
	...
	const char* prog_list = panda_parse_string_opt(args, "prog_list", "", "see Readme");
	printf("prog_list raw: %s\n", prog_list );
	if (0 != strcmp(prog_list, "")) {
		printf("prog_list: %s\n", prog_list);
		if (!parse_prog_list(prog_list, stack_segregation != STACK_THREADED)) {
			return false;
		}
	} else {
		printf("prog_list is unspecified, ignore\n");
	}
